...
\SetKwInOut{Output}{output}
  \Input{grafo $G$ donde cada nodo es un equipo y cada arista es una conexión entre equipos. 
         Asumimos que $G$ es conexo y tiene al menos un ciclo y tiene exactamente $n$ aristas}
  \Output{$<C,R>$, donde $C$ es un conjunto de aristas pertencientes a $E(G)$, que en 
          particular forman un ciclo, y $R$ es un conjunto de aristas incluidas en $E(G)$.
          $C$ y $R$ conectan a todos los nodos de $G$ y son el conjunto de aristas óptimo
		  ...
		  \While{padre $\neq$  nodox.valor}{
				resTemp.pushback(T2[padre]); \\
				padre = padres[T2[padre].valor]; \\
		  }
		  \For{i = 1 to resTemp.size()}{
				\For{j = 1 to T.size()}{
				    si el nodo i está en resTemp y en el árbol T, entonces está en el ciclo: estan[i] = 1
				}
		  }
